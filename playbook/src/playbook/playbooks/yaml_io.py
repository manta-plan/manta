...
def playbook_from_doc(
    ...
    for step_doc in doc.steps:
        ...
        if step_doc.block is not None:
            ...
            steps.append(BlockStep(name=step_doc.name, block=block, inputs=inputs, when=when))
        else:
            child = _nested_playbook(step_doc, loader, catalogue, _chain)
            steps.append(
                NestedPlaybookStep(name=step_doc.name, playbook=child, inputs=inputs, when=when)
            )

    return Playbook(name=doc.name, initial_dims=frozenset(doc.initial_data.dims), steps=steps)
...
def _nested_playbook(
    ...
    if loaded.key in chain:
        ...
        raise PlaybookLoadError(f"step {step_doc.name!r}: circular playbook reference: {loop}")
...
def playbook_to_doc(playbook: Playbook) -> PlaybookDoc:
    ...
    for step in playbook.steps:
        step_doc = StepDoc(
            ...
            playbook=(
                playbook_to_doc(step.playbook) if isinstance(step, NestedPlaybookStep) else None
            ),
            when=(WhenDoc(config=step.when.config, equals=step.when.equals) if step.when else None),
...
def load_config(path: str | Path) -> dict:
    ...
    if not isinstance(raw, dict):
        raise PlaybookLoadError(f"settings file {path} has to be a mapping at the top level")
    return raw
