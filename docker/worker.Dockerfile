# The image behind every playbook-execution container in compose-dev-services.yaml:
# the job containers the docker work pools spawn (one per playbook/block run), the
# Prefect workers that spawn them, and the one-shot provision container. Build
# context is the repo root, since the image needs manta-blocks as well as
# manta-batteries.
#
