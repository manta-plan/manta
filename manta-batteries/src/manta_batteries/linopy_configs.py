...
from manta_blocks.core import ConfigSchema


class LinopyModelSchema(ConfigSchema):
    # TODO: dynamically generate this schema from the linopy Model init signature, so that we don't have to maintain it manually.
    ...
    chunk: int | None = None
    """Chunksize used when assigning data, this can speed up large programs while keeping memory-usage low."""
    ...
    """Whether to automatically mask variables and constraints where bounds, coefficients, or RHS values contain NaN. """

    freeze_constraints: bool = False
    """Whether constraints added to the model should be frozen to the CSR-backed representation by default."""
...
class HighsOptions(ConfigSchema):
    # TODO: dynamically generate this schema from the given Highs version, so that we don't have to maintain it manually.
    threads: int = 1
