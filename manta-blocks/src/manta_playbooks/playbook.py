...
This module holds only the description. Checking it, drawing it, running it, and
deploying it live in `validation`, `graph`, `execution`, and `deploy`.
"""
...
if TYPE_CHECKING:
    ...
    from manta_blocks import BlockDims, Catalogue, DeploymentPlan
    from manta_playbooks.graph import PlaybookGraph
...
class NestedPlaybookStep(BaseModel):
    ...
    def dims(self, config: dict) -> "BlockDims":
        ...
        from manta_blocks import BlockDims
...
class Playbook(BaseModel):
    ...
    def active(self, config: dict) -> "Playbook":
        ...
        deployed, and run can never drift apart.
        """
    ...
    def from_yaml(
        ...
        from manta_playbooks.yaml_io import load_playbook
    ...
    def issues(self, config: dict | None = None) -> "list[PlaybookIssue]":
        ...
        from manta_playbooks.validation import validate_report
    ...
    def to_graph(self, config: dict | None = None) -> "PlaybookGraph":
        ...
        from manta_playbooks.graph import playbook_graph

        return playbook_graph(self, config)

    def to_mermaid(self, config: dict | None = None) -> str:
        """A Mermaid diagram of this playbook."""
        from manta_playbooks.graph import graph_to_mermaid

        return graph_to_mermaid(self.to_graph(config))

    def deployment_plan(self, config: dict) -> "DeploymentPlan":
        """What has to be deployed before this playbook can run."""
        from manta_playbooks.deploy import deployment_plan

        return deployment_plan(self, config)

    def to_flow(self, config: dict, dispatch: bool = False) -> "Flow":
        """This playbook as a Prefect flow, ready to run."""
        from manta_playbooks.execution import build_flow

        return build_flow(self, config, dispatch=dispatch)

    def run(
        self, record: DataRecord, config: dict, dispatch: bool = False
    ) -> DataRecord:
        """Check this playbook and run it."""
        from manta_playbooks.execution import run_playbook_locally

        return run_playbook_locally(self, record, config, dispatch=dispatch)
