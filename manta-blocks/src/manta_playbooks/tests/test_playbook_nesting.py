...
from manta_blocks.tests.fakes import (
    FakeAddsClustered,
    FakeNeedsUpstream,
    FakePassthrough,
...
def test_two_level_nesting_aggregates_into_one_deployment_plan():
    grandchild = Playbook(name="grandchild")
    grandchild.add("leaf", FakePassthrough)

    child = Playbook(name="child")
    child.add_playbook("inner", grandchild)

    outer = Playbook(name="outer")
    outer.add_playbook("mid", child)

    plan = outer.deployment_plan({"mid": {"inner": {"leaf": {}}}})
    assert len(plan.deployments) == 1
    dep = plan.deployments[0]
    assert dep.block == "fake_passthrough"
    assert dep.env == "default"
    assert dep.paths == (("mid", "inner", "leaf"),)


@pytest.mark.slow
def test_nested_playbook_executes_as_a_subflow(initial_record):
    child = Playbook(name="child")
    ...
    result = outer.run(initial_record, config)
