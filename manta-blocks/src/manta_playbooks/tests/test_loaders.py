...
import manta_blocks.tests.fakes  # noqa: F401 - registers the blocks the documents below name
...
def test_a_nested_playbook_can_come_from_somewhere_other_than_a_file():
    ...
    pb = playbook_from_doc(
        outer, loader=InMemoryPlaybookLoader({"memory://child": child})
    )

    assert len(pb.steps) == 1
    assert pb.steps[0].playbook.name == "child"
    pb.validate_playbook(config={"regional": {"inner": {}}})
