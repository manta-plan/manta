...
"""Run a single block from the command line.
    ...
    python -m blocks.run_one <block> --record '{"url": ...}' --output-base <url> \
...
def main(argv: list[str] | None = None) -> int:
    ...
    wired = {
        name: DataRecord.from_dict(value)
        for name, value in json.loads(args.inputs).items()
    }
