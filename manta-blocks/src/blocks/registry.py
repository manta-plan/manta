...
from blocks.core import BlockDims, ConfigSchema, MantaBlock
from blocks.environments import EnvironmentSpec, current_env
...
# The blocks shipped with Manta need PyPSA, which is not in every environment.
# Recording where they live, rather than importing them here, keeps this module
# usable in environments that cannot import them.
_LAZY: dict[str, str] = {
    "cluster_time": "blocks.library.time_cluster:ClusterTime",
    "overnight_capacity_expansion": (
        "blocks.library.overnight_capacity_expansion:OvernightCapacityExpansion"
    ),
    "myopic_capacity_expansion": (
        "blocks.library.myopic_capacity_expansion:MyopicCapacityExpansion"
    ),
    "rolling_horizon_dispatch": (
        "blocks.library.rolling_horizon_dispatch:RollingHorizonDispatch"
    ),
}
