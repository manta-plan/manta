...
Which steps run is decided once, up front, using the same question the rest of the
system asks. A playbook can therefore never run something that was not checked, or
skip something that was.
"""

from typing import Protocol

from blocks import BlockSpec, DataRecord
from playbooks.playbook import BlockStep, Playbook, Step, child_config
...
class LocalStepRunner:
    ...
    def run_block(
        ...
        *,
        ...
        return block_cls(block_cls.merge_config(config, inputs)).run(
            record, output_base
        )
...
def execute_playbook(
    ...
    *,
    ...
    """Check `playbook` and run it, returning the final step's record.
    ...
    return _execute(
        playbook, record, config, output_prefix, runner or LocalStepRunner(), {}
    )
