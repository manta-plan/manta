...
"""The playbooks that ship with Manta, and how to find them.
...
from playbooks.yaml_io import PlaybookDoc, PlaybookLoadError, parse_doc
    ...
    for entry in sorted(root.iterdir(), key=lambda e: e.name):
        ...
        if doc.name in found:
            raise PlaybookLoadError(
                f"two library playbooks both call themselves {doc.name!r}"
            )
        found[doc.name] = LibraryPlaybook(
            doc=doc, default_config=_default_config(root, entry.name)
        )
    return found
