...
def test_a_playbook_read_from_file_validates_against_its_settings():
    pb = load_playbook(FIXTURES / "fake_playbook.yaml")
    pb.validate_playbook(load_config(FIXTURES / "fake_config.yaml"))
...
def test_a_playbook_survives_a_round_trip_through_its_document():
    ...
    assert [s.block.name for s in rebuilt.steps] == [
        s.block.name for s in original.steps
    ]
