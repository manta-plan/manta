...
from manta_blocks import registry
...
from manta_blocks.tests.fakes import FakeNeedsUpstream
...
def test_merge_combines_catalogues_from_different_environments():
    ...
    elsewhere = Catalogue(
        blocks={
            "far_away": BlockDescription(
                name="far_away", env="solver", module="somewhere:FarAway"
            )
        }
    )
...
def test_a_block_that_cannot_be_imported_here_is_left_out(monkeypatch):
    ...
    monkeypatch.setitem(
        registry._LAZY, "needs_another_env", "not_installed_anywhere:Block"
    )
