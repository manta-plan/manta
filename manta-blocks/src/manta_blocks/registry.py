...
in the system - enough to draw it, offer its settings, and check how it is wired -
without needing an environment that can import them all.

Block libraries (such as `manta_batteries`) announce their blocks by calling
...
from manta_blocks.core import BlockDims, ConfigSchema, MantaBlock
...
class BlockUnavailableError(Exception):
    ...
    Usually this means the pixi environment that provides the block is not the one
    currently active.
    """
...
class BlockDescription(BaseModel):
    ...
    This is what a playbook needs in order to place a block in a diagram, offer its
    settings, and check that it is wired up correctly.
    """
    ...
    def for_block(cls, block: type[MantaBlock], name: str) -> "BlockDescription":
        return cls(
            ...
            config_schema=_tag_wired_settings(
                block.CONFIG.model_json_schema(), block.INPUTS
            ),
        )
...
class BlockSpec:
    ...
    playbook can be built, checked, drawn, and deployed from a process that cannot
    import the blocks it names. Actually running a block needs the real class, and
    asking for one that is not available says so plainly.
    """
    ...
    @property
    def label(self) -> str:
        """A short name for this block in messages and diagrams."""
...
# Blocks known by name and location but not yet imported. Block libraries fill this
# via `register_lazy`, so that a library's blocks can be looked up in environments
# that could never import them (most blocks need a solver stack such as PyPSA).
_LAZY: dict[str, str] = {}

BLOCK_SOURCES_ENV_VAR = "MANTA_BLOCK_SOURCES"
"""Comma-separated modules whose import registers blocks, e.g. `manta_batteries`."""
...
def register_lazy(name: str, locator: str) -> None:
    ...
    if name in _REGISTRY or name in _LAZY:
        raise BlockRegistrationError(
            f"A block is already registered under the name {name!r}"
        )
    _LAZY[name] = locator
...
def get_block(name: str) -> type[MantaBlock]:
    ...
    if name in _LAZY:
        ...
        try:
            ...
            raise BlockUnavailableError(
                f"Block {name!r} could not be imported ({exc}). Is the pixi "
    ...
    raise BlockNotFoundError(
        f"No block registered under {name!r}. Available blocks: "
        f"{sorted(available_blocks())}"
    )
...
def resolve_block(name: str, catalogue: Catalogue | None = None) -> BlockSpec:
    ...
    If the block cannot be imported here - or was never even registered here, as in a
    process that only knows blocks from a catalogue file - but `catalogue` describes
    it, the description is used instead. That is enough to build, check, draw, and
    deploy a playbook; only running the block itself needs the real class.
    """
    try:
        return describe_block(get_block(name), name)
    except (BlockNotFoundError, BlockUnavailableError):
...
def catalogue() -> Catalogue:
    ...
    return Catalogue(
        generated_in_env=current_env(), blocks=blocks, environments=environments
    )
