...
Decided changes that are simply not implemented yet live in
[09](09-code-changes.md) instead.
...
because a record is currently a path to a PyPSA netCDF file.
Manta's data layer is an
...
1. **Where records live.** Local paths do not work once steps run in different
   containers.
This must become object-store URLs.
...
2. **What a record *is*.** The security review specifies DuckDB over S3 for model
   ...
   because PyPSA cannot yet store a difference.
These need reconciling — a record
...
input/output — how a block reads and writes one.
The contract between them is the URL
scheme plus credentials.

It blocks Kubernetes entirely.
The implementation delta is in
[09](09-code-changes.md#records-must-live-in-the-object-store); what remains open here
is question 2.

### Prefect result persistence across processes

The orchestrator reads each step's return value across a process boundary.
That
requires Prefect result persistence configured with storage both sides can reach, and
it is not configured today.
The proof of concept passes because its tests run in one
process.

It works locally and breaks the moment execution is genuinely distributed, so it
should be verified on the Compose stack with separate worker processes before anything
else is built on top of it.
See
[09](09-code-changes.md#configure-prefect-result-persistence).

### Resource requirements are not modelled

No block declares CPU, memory or wall time.
On Kubernetes this is not optional.

The declaration belongs on the block, because only its author knows what it needs.
The
...
as a container image, which Manta then executes inside its own infrastructure.
At M4
that infrastructure also handles client Confidential data.

Nothing in the current design addresses this.
The questions:

- **Who may publish a block?** Is the catalogue curated by OET, or can a deployment
  add third-party blocks?
On-premise clients will certainly want the latter.
...
- **How is data scoped?** A block sees whatever record it is handed.
Credentials
...
This should feed the M3 threat-modelling exercise.
It is not urgent for M1–M3 (public
...
Run state and logs stay in Prefect — settled.
But a completed run produces a URL
...
state, while continuing to read status and logs live.
...
**Open:** who writes it, given nothing currently observes run completion?
Options: on
...
- **Pin:** a playbook authored a year ago still validates and runs against the blocks
  it was written for.
Requires keeping old catalogues and old deployments alive, and
...
Reproducibility is listed as a quality goal for v1, which argues for pinning.
This
...
The `blocks` repository ships example playbooks as YAML.
Manta stores user-authored
playbooks in its database.
Are these the same kind of thing?

**Open:** does Manta ship a library of curated starter playbooks — imported from
`blocks` at release time, or authored in Manta and versioned somewhere?
The roadmap
speaks of a "simple, modular playbook system" at MVP advancing to "user-defined
playbooks", which implies both exist.
...
already-dispatched children continue, orphaned.
Running it on a stable process pool
makes this unlikely rather than impossible.

**Open:** is that acceptable, or does the orchestrator need to be resumable — able to
pick up a partially-completed playbook and continue from the last finished step?
The
...
Every run in a deployment currently shares one pool per environment.
For OET's SaaS
...
priority queues.
Prefect supports concurrency limits and work queues with priorities
within a pool, which is probably sufficient and considerably simpler than pool
proliferation.
Not needed before M3's guest-access model, which already restricts size
and run limits.
...
`when` clauses inspect settings only, never the data.
...
without giving up up-front validation?
No answer is needed yet; it should be recorded
...
nothing surfaces it.
Prefect can emit automations on late runs; Manta could surface
queue age on the run endpoint. **Open:** which, and what the user is told.

### Observability beyond Prefect

Prefect covers run state and logs.
It does not cover the API, the database, or the
